        ...
        <form id="kc-passwd-update-form" onsubmit="login.disabled = true; return true;" action="${url.loginAction}" method="post" novalidate="novalidate">
            ...
            <@buttons.actionGroup horizontal=true>
                <#if isAppInitiatedAction??>
                    <@buttons.button id="kc-submit" name="login" label="doSubmit" class=["btn", "btn-primary"]/>
                    <@buttons.button id="kc-cancel" label="doCancel" name="cancel-aia" class=["btn", "btn-secondary"]/>
                <#else>
                    <@buttons.button id="kc-submit" name="login" label="doSubmit" class=["btn", "btn-primary", "w-100"]/>
                </#if>
            </@buttons.actionGroup>
        </form>
