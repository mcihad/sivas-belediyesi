...
<!-- template: webauthn-authenticate.ftl -->
    ...
    <#elseif section = "form">
        <div id="kc-form-webauthn">
                ...
                <#if authenticators??>
                    ...
                    <#if shouldDisplayAuthenticators?? && shouldDisplayAuthenticators>
                        <#if authenticators.authenticators?size gt 1>
                            <p class="mb-3">${kcSanitize(msg("webauthn-available-authenticators"))?no_esc}</p>
                        </#if>

                        <ul class="list-group mb-3" role="list">
                            <#list authenticators.authenticators as authenticator>
                                <li class="list-group-item d-flex align-items-center">
                                    <div id="kc-webauthn-authenticator-item-${authenticator?index}" class="d-flex align-items-center w-100">
                                        <div class="me-3">
                                            <div class="text-primary">
                                            <#switch authenticator.transports.iconClass>
                                                <#case "kcWebAuthnBLE">
                                                    <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                                         viewBox="0 0 217.499 217.499" xml:space="preserve" width="24" height="24"><path
                                        ...
                                        <div class="flex-grow-1">
                                            ...
                                            <div id="kc-webauthn-authenticator-createdlabel-${authenticator?index}" class="text-muted small">
                                                <i>${kcSanitize(msg('webauthn-createdAt-label'))?no_esc}</i>
                                            </div>
                                            <div id="kc-webauthn-authenticator-created-${authenticator?index}" class="text-muted small">
            ...
            <@buttons.actionGroup>
                <@buttons.button id="authenticateWebAuthnButton" label="webauthn-doAuthenticate" class=["btn-primary","w-100"]/>
            </@buttons.actionGroup>
        </div>
    <script type="module">
